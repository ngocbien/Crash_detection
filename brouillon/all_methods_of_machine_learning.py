...
DICT_DATASET = dict_dataset.dict_dataset
PLOT = True
if PLOT:
...
def processing_data_by_agreated_and_using_kalman_filter(raw_data, M = 1):
    ...
    if M > 1:
        ...
        for index in range(N):
            this_row = raw_data[index, :]
            for i in range(M-1):
                this_row += raw_data[index+i+1, :] / M
            if index == 0:
...
def get_list_of_g(estimated_data_acc):
    ...
    for index, line in enumerate(estimated_data_acc):
        ...
        if vector_g >= 0:
            ...
            sign_g = -1
        value_g = np.sqrt(sum(np.square(line)))*sign_g
        list_.append(value_g)
        mean_list = sum(list_)/len(list_)
    return list_ - mean_list

def get_list_of_g_by_average(estimated_data_acc, M):
    """
    :param data: is 3D accelerometer clean data, with a given frequen
    M: is number of lines to reduce
    :return: numpy array data of acceleration or decceleration
